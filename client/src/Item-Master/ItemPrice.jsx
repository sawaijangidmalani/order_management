...
function ItemPrice({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (isEditing && editItemId) {
        await axios.put(
          `https://order-management-tgh3.onrender.com/itemPrice/updateItemPrice/${editItemId}`,
          payload
        );
        toast.success("Item price updated successfully!");
      } else {
        await axios.post(
          "https://order-management-tgh3.onrender.com/itemPrice/addItemPrice",
          payload
        );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `https://order-management-tgh3.onrender.com/itemPrice/deleteItemPrice/${id}`
      );
      await fetchItemPrices(selectedItemId);
